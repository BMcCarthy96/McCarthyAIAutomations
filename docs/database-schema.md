...
- **support_requests** — Mostly **one-to-many** from clients (portal). Rows may also have **no** `client_id` (public marketing submissions) with `requester_name` / `requester_email` instead. Optionally linked to a project via `project_id`. Public consultation rows can use **`lead_follow_up_eligible`** / **`follow_up_sent_at`** / **`lead_follow_up_suppressed`** for the booking follow-up automation (see migrations `20250317170000_lead_follow_up_support_requests.sql`, `20250317180000_lead_follow_up_suppressed.sql`). **AI Lead Engine (MVP)** adds nullable **`ai_*`** columns on the same table (`ai_lead_analysis_status`, summary, temperature, urgency, suggested reply, etc.); see migration `20260404120000_support_requests_ai_lead_analysis.sql`.
